...
struct ModelFile { url_path: &'static str, local: &'static str }
const FILES: &[ModelFile] = &[
    ModelFile { url_path: "onnx/encoder_model_int8.onnx",  local: "models/encoder.onnx"  },
    ModelFile { url_path: "onnx/decoder_model_int8.onnx",  local: "models/decoder.onnx"  },
    ModelFile { url_path: "tokenizer.json",                 local: "models/tokenizer.json" },
];
...
fn download_models() -> Result<()> {
    ...
    for f in FILES {
        ...
        if dest.exists() { eprintln!("  ✓ {} (cached)", f.local); continue; }
...
fn run_encoder(session: &mut Session, input_ids: &[i64]) -> Result<(Vec<f32>, Vec<i64>)> {
    ...
    let ids_t  = Tensor::from_array(([1i64, seq], input_ids.to_vec()))
        .context("encoder ids")?;
    let mask_t = Tensor::from_array(([1i64, seq], vec![1i64; input_ids.len()]))
        .context("encoder mask")?;

    let outputs = session.run(ort::inputs![ids_t, mask_t]).context("encoder run")?;
...
fn greedy_decode(
    ...
    for _ in 0..MAX_NEW {
        ...
        let mask_t = Tensor::from_array(([1i64, src_len as i64], vec![1i64; src_len]))
            .context("mask")?;
        let ids_t  = Tensor::from_array(([1i64, dec_len], tokens.clone()))
            .context("ids")?;
        let hid_t  = Tensor::from_array(
            ([1i64, src_len as i64, hidden_size], enc_hidden.to_vec())
        ).context("hidden")?;
        ...
        let next_id = last.iter().enumerate()
            .max_by(|(_, a), (_, b)| a.partial_cmp(b).unwrap())
            .map(|(i, _)| i as i64)
            .unwrap_or(0);

        if next_id as u32 == EOS_ID { break; }
        tokens.push(next_id);
    }
...
fn main() -> Result<()> {
    ...
    for src in &sentences {
        ...
        let enc = tokenizer.encode(*src, false)
            .map_err(|e| anyhow::anyhow!("{e}"))?;
        ...
        let result  = tokenizer.decode(&out_ids, true)
            .map_err(|e| anyhow::anyhow!("{e}"))?;
